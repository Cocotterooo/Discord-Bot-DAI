...
from config import (
    ...
    tickets_embed,
    verification_embed,
    support_and_verification_embed,
)
...
async def create_ticket(interaction: discord.Interaction):
    ...
    existing_ticket_channel = discord.utils.get(
        guild.channels, name=f"🎫⦙{interaction.user.name}"
    )
    if existing_ticket_channel:
        return await interaction.response.send_message(
            "<:no:1288631410558767156> Ya tienes un ticket abierto.", ephemeral=True
        )
    ...
    await ticket_channel.set_permissions(
        interaction.user, read_messages=True, send_messages=True, attach_files=True
    )
    await ticket_channel.set_permissions(
        guild.get_role(DAI_MEMBER_ROLE_ID),
        read_messages=True,
        send_messages=True,
        attach_files=True,
    )
    ...
    await interaction.response.send_message(
        f"<:correcto:1288631406452412428> Ticket creado: {ticket_channel.mention}",
        ephemeral=True,
    )
...
async def verify(interaction: discord.Interaction):
    ...
    if verified_role in interaction.user.roles:
        return await interaction.response.send_message(
            "<:no:1288631410558767156> Ya estás verificado.", ephemeral=True
        )
    if existing_ticket_channel:
        return await interaction.response.send_message(
            "<:no:1288631410558767156> Ya tienes un ticket de verificación abierto.",
            ephemeral=True,
        )
    ...
    await verify_channel.send(interaction.user.mention, 
                              embed=embed, 
                              view=VerificationView(interaction.user)
    )

    await interaction.response.send_message(
        f"<:correcto:1288631406452412428> Canal de verificación creado: {verify_channel.mention}",
        ephemeral=True,
    )
...
async def deny_verification(interaction: discord.Interaction):
    ...
    if support_role not in interaction.user.roles:
        return await interaction.response.send_message(
            "<:no:1288631410558767156> No tienes permiso para usar este botón.", ephemeral=True
        )
    await interaction.channel.delete()
...
async def accept_verification(interaction: discord.Interaction):
    ...
    if member is None:
        return await interaction.response.send_message(
            "<:no:1288631410558767156> No se encontró al usuario correspondiente.",
            ephemeral=True,
        )
...
def support_and_verification(bot):
    @bot.tree.command(
        name="setup_soporte_verificacion", description="Enviar el embed principal"
    )
